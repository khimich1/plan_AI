...
Последнее обновление: 2026-09-04 (прогон archive+production — архив и производство).
...
| A4 | kp | High | open | P1 | Толстый API-слой: `commercial.py` разобран; remainder `production.py` (698 строк, 26 routes) | `production.py` wc=698; `build_plan_from_filters` :104–180; day-documents :434–522 bypass ProductionService; SGP CRUD :535–608; helpers :628–678 | 2026-09-04 | |
| A5 | kp | High | open | P1 | Сервисы обходят repository, raw SQL | `sgp_service.py:78–89`, `delivery_schedule_service.py:494–500`, `kp_readiness_service.py:49–114`; нет *sgp* / *delivery* / *readiness* repos | 2026-09-04 | |
...
| A11 | kp | Medium | open | — | ArchiveService god-orchestrator | `archive_service.py` 868 строк, 30 methods; lazy-coupling :124–146 CommercialWorkflowService, :491–494 SgpService, :588–592 KpReadinessService | 2026-09-04 | |
...
| A22 | kp | High | open | P1 | God-сервис `ProductionCompletionService` + raw sqlite3 | `production_completion_service.py` wc=702; sqlite3 :4, :81–85, :125–128, :309, :515 | 2026-09-04 | |
...
| S8 | platform | Medium | open | — | Утечка деталей ошибок в HTTP | `production.py:141`, `:174`, `:203`, `:209`, `:696`, `:147`, `:406`, `:428`, `:580`, `:605`; `archive.py:323`, `:347`, `:373`, `:407`, `:413`, `:491`, `:537`; `delivery_schedule.py:53`, `:76`, `:92`, `:128`, `:156` — `str(exc)` via `raise_unprocessable_client_error` | 2026-09-04 | |
...
| Q8 | kp | Medium | open | — | God-hook `useCreatePlanWizardState` | wc=997; `eslint-disable` :256, :368 | 2026-09-04 | |
| Q9 | kp | Medium | open | — | God-component `OfferDetailsDrawer` | 1091 строк (было 1066) | 2026-09-04 | |
| Q10 | kp | Medium | open | — | Слабая типизация production API | 9 routes без `response_model`: `production.py:87`, `:95`, `:220`, `:266`, `:321`, `:385`, `:525`, `:611`, `:619` | 2026-09-04 | |
...
| Q12 | kp | Medium | open | — | ArchiveService скрывает частичные сбои | `:487–500`, `:555–556`, `:593–594`, `:789–791` swallow; `move_to_production` :247–253 re-raises | 2026-09-04 | |
...
| Q18 | kp | Low | open | — | Подавление `react-hooks/exhaustive-deps` в 9 файлах | kp subset: `MoveToProductionDialog.tsx:87`, `DeliveryScheduleDialog.tsx:92`, `useCreatePlanWizardState.ts:256`, `:368` | 2026-09-04 | |
...
| Q23 | kp | Medium | open | — | Расхождение оценки дорожек: archive `round` vs production FE `ceil` | `archive_service.py:289–290` vs `productionEstimate.ts:24–25`; 101 м → 2 vs 1 | 2026-09-04 | |
| Q24 | kp | Medium | open | — | Copy-paste day-document handlers: мёртвый raise после raise_not_found | `production.py:480–489` breakdown, `:507–516` formovka vs schema `:447–458` | 2026-09-04 | |

## Счётчики (прогон archive+production, 2026-09-04)
...
| auth | 0 | 0 |
| **все open Critical/High** | **2 P0** | **14 P1** (A4, A5, A6, A7, S3, A16–A20, S16, Q19, Q20, A22) |

A18 = gsm LibreOffice+DoS (старые gsm A3+S1). Не плодить второй ID.
A21 (Medium), Q21 (Low) — не входят в P1.

Следующий свободный ID: **A23**, **S17**, **Q25**.
