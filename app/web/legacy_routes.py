...
def _check_draft_ownership(draft_id: str, user: dict) -> None:
    """Lazy import so tests can monkeypatch ``app.web.router.check_draft_ownership``."""
    import app.web.router as web_router

    web_router.check_draft_ownership(draft_id, user)


def legacy_new_offer_error_response(request: Request, message: str) -> RedirectResponse | JSONResponse:
...
def redact_saved_offer_if_forbidden(draft: dict, user: dict) -> dict:
...
_LEGACY_LOGOUT_HINT = (
    "GET /web/logout is deprecated. Use POST /api/v1/auth/logout with a valid CSRF token."
)
...
def web_logout() -> JSONResponse:
    response = JSONResponse(
        status_code=405,
        content={"detail": _LEGACY_LOGOUT_HINT},
        headers={"Allow": "POST"},
    )
...
async def new_offer_submit(
    ...
    try:
        ...
        return legacy_new_offer_error_response(request, "Выберите менеджера.")

    try:
        parsed_discount = float((discount_percent or "0").replace(",", "."))
    except ValueError:
        return legacy_new_offer_error_response(request, "Скидка должна быть числом.")
    ...
    try:
        ...
        return legacy_new_offer_error_response(request, msg)
    ...
    try:
        ...
        return legacy_new_offer_error_response(request, str(exc))
...
def offer_draft_page(
    ...
    try:
        _check_draft_ownership(draft_id, user)
    except HTTPException:
