...
from core.commercial_pricing import lookup_march_price
from core.exceptions import PriceNotFoundError
from core.march_line_parser import merge_march_lines, parse_march_line
from core.march_text_normalizer import normalize_march_order_text


@dataclass
class CommercialMarchPreviewResult:
    normalized_text: str
...
class CommercialMarchService:
    """Build march order_data from normalized text + march_prices lookup."""
    ...
    def generate_preview(
        ...
        *,
    ...
    ) -> CommercialMarchPreviewResult:
        normalized = normalize_march_order_text(text)
        raw_lines = normalized.normalized_lines or [
            part.strip() for part in (text or "").splitlines() if part.strip()
        ]

        parsed_lines = [
            parse_march_line(line, default_grade=default_grade) for line in raw_lines
        ]
        unparsed_lines = [
            raw_lines[idx]
        ...
        merged = merge_march_lines(parsed_lines, default_grade=default_grade)
        ...
        return CommercialMarchPreviewResult(
