# -*- coding: utf-8 -*-
"""Геометрические ключи вторичных резов и слияние вариантов по parent."""
...
def canonical_target_order_key_tok(tok: object | None) -> tuple | None:
...
def secondary_merge_key(tok: object | None, geom_key: tuple[float, int]) -> tuple[tuple | None, tuple[float, int]]:
    return (canonical_target_order_key_tok(tok), geom_key)


def merge_atomic_secondaries_by_shared_parent(
    *,
    ...
    for pid, bucket in list(secondary_cuts_by_parent.items()):
        ...
        for variant in bucket:
            ...
            if gk is None:
                ...
                continue
            mk = secondary_merge_key(variant.get("target_order_key"), gk)
...
def append_parent_variants_to_secondary_cuts_info(
    *,
...
def plan_stock_width_mm(plan: dict) -> int:
...
def extract_orphan_secondaries_as_synthetic_primary_cuts(
    *,
    secondary_cuts_info: dict,
    plan: dict,
    plate_width_mm: int,
    normalize_load_code: Callable[..., Any] | None = None,
) -> list[dict]:
    """
    ...
    for gk in list(secondary_cuts_info.keys()):
        ...
        for v in secondary_cuts_info[gk]:
            ...
            if tok and len(tok) > 2:
                lc = _nlc(tok[2], default=8)
            else:
                lc = _nlc(row.get("load_code") or assign.get("load_code", 8))
