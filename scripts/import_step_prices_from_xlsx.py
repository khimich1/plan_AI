#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Импорт прайса лестничных ступеней (ЛС) из Excel в pb.db (таблица step_prices)."""
...
from core.step_price_db import import_step_prices_from_xlsx, parse_step_price_rows_from_xlsx


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Загрузить прайс лестничных ступеней из Excel в pb.db (INSERT OR REPLACE)."
    )
    ...
    preview = parse_step_price_rows_from_xlsx(str(xlsx_path), preferred_sheet=args.sheet)
    if not preview:
    ...
    print(f"БД:    {args.db}")
    print(f"Готово: {inserted} строк ({unique_marks} уникальных марок ЛС)")
    return 0
