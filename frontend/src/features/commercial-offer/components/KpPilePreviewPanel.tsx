      ...
      <div style={{ display: "grid", gap: "0.75rem" }}>
        ...
        {onApplyGradeToAll && rows.length > 0 && (
          ...
          >
            ...
            >
              {PILE_GRADE_CODES.map((code) => (
                <option key={code} value={code}>
                  {formatPileGradeLabel(code)}
                </option>
              ))}
            </select>
            <Button
              type="button"
              variant="secondary"
              disabled={isUpdatingGrades || !isPileGradeCode(bulkGrade)}
        ...
        {rows.length === 0 ? (
          <div style={{ color: "#667085" }}>Список пуст — распознайте сваи.</div>
        ) : (
                    ...
                    <tr key={`${row.mark}-${row.concrete_grade}-${index}`}>
                      ...
                      <td style={{ padding: "0.55rem 0.65rem", borderBottom: "1px solid #f2f4f7" }}>
                        {onLineGradeChange ? (
                          ...
                          formatPileGradeLabel(row.concrete_grade)
                        )}
                      </td>
...
export { buildPileLinesFromOrderData };
