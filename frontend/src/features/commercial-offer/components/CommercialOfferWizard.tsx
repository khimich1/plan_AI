...
import { mapLegacyWizardStep, WIZARD_STEP_ORDER, wizardStepIndex } from "@/features/commercial-offer/lib/wizardStepOrder";
...
export const CommercialOfferWizard = () => {
  ...
  const handleApplyAi = async () => {
    ...
    if (!currentDraft?.draft_id) {
      ...
      setStepError("Сначала распознайте список плит, затем используйте помощника.");
    ...
    if (editedText && editedText !== originalBatchText) {
      ...
      try {
        ...
        dispatch({
          ...
          batchCount: draft.metadata.plate_batches?.length ?? 0,
  ...
  const handleSidebarStepClick = (step: WizardStepId) => {
    ...
    if (!canNavigateToStep(step)) {
      ...
      if (step === "client" && (hasUnresolvedWidePlates || state.pendingBatchReview)) {
