...
export const KpMarchPreviewPanel = ({
  draft,
  normalizedText,
  ...
  onLineGradeChange,
  lineRowHandlers,
}: KpMarchPreviewPanelProps) => {
  const rows = useMemo(() => buildMarchPreviewRows(draft), [draft]);
    ...
    >
      <div style={{ display: "grid", gap: "0.75rem" }}>
        ...
        {normalizedTextChanged && (
          <Alert tone="info">Изменён список маршей — нажмите «Список верен» для пересчёта состава.</Alert>
        )}
        ...
        {onApplyGradeToAll && rows.length > 0 && (
          ...
          >
            ...
            >
              {MARCH_GRADE_CODES.map((code) => (
                <option key={code} value={code}>
                  {formatMarchGradeLabel(code)}
                </option>
              ))}
            </select>
            <Button
              type="button"
              variant="secondary"
              disabled={isUpdatingGrades || !isMarchGradeCode(bulkGrade)}
        ...
        {rows.length === 0 ? (
          <div style={{ color: "#667085" }}>Список пуст — распознайте марши.</div>
        ) : (
                    ...
                    <tr key={`${row.mark}-${row.concrete_grade}-${index}`}>
                      ...
                      <td style={{ padding: "0.55rem 0.65rem", borderBottom: "1px solid #f2f4f7" }}>
                        {onLineGradeChange ? (
                          ...
                          formatMarchGradeLabel(row.concrete_grade)
                        )}
                      </td>
...
export { buildMarchLinesFromOrderData };
