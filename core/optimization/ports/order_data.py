...
class PlateOrderDataPort(Protocol):
    ...
    Implemented in-process by ``ConfigModulePlateOrderAdapter`` wrapping
    ``core.config_and_data``.
    """

    def normalize_load_code(self, value: Any, default: int = 8) -> int | float:
        """Same contract as ``core.config_and_data.normalize_load_code``."""
        ...
...
class ConfigModulePlateOrderAdapter:
    """Thin adapter: forwards normalization to ``config_and_data``; 1D costs match previous defaults."""

    __slots__ = ("_cfg",)

    def __init__(self, cfg: ModuleType) -> None:
        self._cfg = cfg

    def normalize_load_code(self, value: Any, default: int = 8) -> int | float:
        return self._cfg.normalize_load_code(value, default)

    def one_d_plate_unit_price_rub(self) -> int:
        return _LEGACY_1D_PLATE_UNIT_PRICE_RUB

    def one_d_long_cut_cost_rub(self) -> int:
        per_m = float(getattr(self._cfg, "LONG_CUT_PRICE_PER_M", 460.0))
        return int(round(per_m * _LEGACY_1D_LONG_CUT_FACTOR_M))


def default_plate_order_port() -> PlateOrderDataPort:
    """Composition-root default: live ``core.config_and_data`` module."""
    from core import config_and_data as cfg

    return ConfigModulePlateOrderAdapter(cfg)
