...
- **Stack:** Python (FastAPI, Pydantic v2, `core`/доменные модули, визуализация, Telegram-бот); отдельно React + Vite + TypeScript во `frontend/` **без настроенного JS/TS test runner в `package.json`**.
- **Test framework:** **pytest** (версия в репозитории не зафиксирована в `requirements.txt` — при необходимости уточняй у разработчика / окружение).
- **File naming:** преимущественно **`test_*.py`** в каталоге `tests/` (не `*_test.py`). Не все файлы в `tests/` — автотесты: часть — проверочные/экспериментальные скрипты.
- **File location:** **`tests/`** у корня проекта; тесты не колокейтятся с исходниками по умолчанию.
- **Mock approach:**
  - **`unittest.mock`**: `MagicMock`, `patch`;
  - **`pytest.MonkeyPatch`**: env, `monkeypatch.setattr` (в т.ч. подмена методов репозиториев);
  - **FastAPI:** `TestClient` из `fastapi.testclient`, `create_app()`, **`dependency_overrides`** для сервисов; при работе с настройками — **`get_settings.cache_clear()`** после смены env;
  - для кода с глобальным состоянием вроде **`core.config_and_data`** — локальные helper'ы с **сохранением/восстановлением** мутабельных структур в рамках теста;
  - **`pytest.raises`**, **`pytest.approx`**, **`pytest.mark.parametrize`**, **`pytest.importorskip`**, **`pytest.skip`** — по месту.
- **Test utilities:** общего **`conftest.py` нет**; вспомогательные функции и **`@pytest.fixture` / `autouse`** обычно объявляются в файле теста; при необходимости — приватные хелперы с префиксом `_`.
- **Example reference:** ориентироваться на `tests/test_archive_endpoints.py` (FastAPI + TestClient + overrides + monkeypatch), `tests/test_commercial_web_flow.py` (web flow + fixtures), `tests/test_layout_secondary_unmatched_parent_user_list.py` (интеграция `core` / сервисов / viz с сохранением состояния cfg).
