...
bank_dir = PROJECT_ROOT / 'банк знаний'
...
if bank_dir.exists():
    for name in os.listdir(bank_dir):
        ...
        if name.lower().endswith('.xlsx') and 'нов' in name.lower() and 'цен' in name.lower():
            PRICE_XLSX_PATH = bank_dir / name
            break

if not PRICE_XLSX_PATH or not PRICE_XLSX_PATH.exists():
...
# Читаем Excel
try:
    all_sheets = pd.read_excel(str(PRICE_XLSX_PATH), sheet_name=None)
    ...
    count = import_from_xlsx(str(PRICE_XLSX_PATH), PRICE_DB_PATH, preferred_sheet)
