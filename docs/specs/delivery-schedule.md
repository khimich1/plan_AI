...
| GET | `/template` | Скачать XLSX-шаблон с марками и количествами из этого КП |
...
`core/delivery_schedule_xlsx.py`: `build_template(path, plates=)` /
...
Шаблон предзаполняет «Марка» и «Кол-во» из `kp_plates` выбранного КП;
партии и даты менеджер дописывает сам, затем POST `/import`.
Матчинг марки → позиция КП: точное по `plate_name`, иначе строка уходит в
`unmatched_rows` с причиной. Строки шаблона без партии и дат (ещё не
заполнены) пропускаются. Даты: `ДД.ММ.ГГГГ` (парсер как
`core/execution_terms` по стилю, строгий).

Дополнение (2026-08-22): двухполосный шаблон (сверху уже в поставках,
ниже остаток, с видимым разделителем). В открытом редакторе файл
собирается на клиенте из черновика; `GET /template` отдаёт тот же макет
от сохранённого графика. См. `ai_docs/specs/delivery-schedule-remainder-template.md`.
